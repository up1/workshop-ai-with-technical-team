...
if __name__ == "__main__":
    ...
    # 2.4 Compress the relevant data
    compressor = LLMChainExtractor.from_llm(llm)
    compression_retriever = ContextualCompressionRetriever(base_compressor=compressor, base_retriever=relevant_data_db.as_retriever(search_kwargs={"k": 4}))
    
    # 3. Generate response
    response = generate_response(compression_retriever , question)
